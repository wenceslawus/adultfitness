<template>
  <q-modal
    v-model="opened"
  >
    <q-modal-layout>
      <q-toolbar color="secondary" slot="header">
        <q-btn
          flat
          type="a"
          dense
          v-close-overlay
          icon="keyboard_arrow_left"
          :label="$t('video.close')"
        />
        <q-toolbar-title v-text="$t('video.share')"/>
      </q-toolbar>

      <div class="layout-padding">
        <div class="container">
          <p class="q-pt-md q-px-md q-ma-none"  v-text="$t('general.share')" />
          <socialSharing
            v-if="opened && currentVideo"
            class="q-pa-md social-sharing"
            style="padding: 0 100px"
            :url="url"
            :title="(currentVideo)?currentVideo.title:''"
            :description="(currentVideo)?currentVideo.content:''"
            quote=""
            hashtags="adultfittness.com"
            twitter-user=""
            inline-template
          >
            <div>
              <q-btn type="a" flat>
                <network network="twitter">
                  <svg viewBox="0 0 60 60" preserveAspectRatio="xMidYMid meet" focusable="false" class="style-scope yt-icon"><g class="style-scope yt-icon">
                    <g fill="none" fill-rule="evenodd" class="style-scope yt-icon">
                      <path d="M28.486325 59.969298c-6.636404-.569063-11.56302-2.326956-16.321973-5.823932C4.443764 48.472116 0 39.646792 0 29.986934 0 15.11156 10.506778 2.798388 25.274412.36718c6.028107-.992411 12.703853.049265 18.28794 2.85363 13.576275 6.818095 19.7813 22.541053 14.64267 37.103159-3.527955 9.997705-12.789708 17.617785-23.391072 19.244938-2.085625.320112-5.065149.508645-6.327625.400391z" fill="#1DA1F2" fill-rule="nonzero" class="style-scope yt-icon"></path>
                      <path d="M45.089067 17.577067c-.929778.595555-3.064534 1.460977-4.117334 1.460977v.001778C39.7696 17.784 38.077156 17 36.200178 17c-3.645511 0-6.6016 2.956089-6.6016 6.600178 0 .50631.058666 1.000178.16711 1.473778h-.001066c-4.945066-.129778-10.353422-2.608356-13.609244-6.85049-2.001778 3.46489-.269511 7.3184 2.002133 8.72249-.7776.058666-2.209067-.0896-2.882844-.747023-.045156 2.299734 1.060622 5.346845 5.092622 6.452267-.776533.417778-2.151111.297956-2.7488.209067.209778 1.941333 2.928355 4.479289 5.901155 4.479289C22.46009 38.565156 18.4736 40.788089 14 40.080889 17.038222 41.929422 20.5792 43 24.327111 43c10.650667 0 18.921956-8.631822 18.4768-19.280356-.001778-.011733-.001778-.023466-.002844-.036266.001066-.027378.002844-.054756.002844-.0832 0-.033067-.002844-.064356-.003911-.096356.9696-.66311 2.270578-1.836089 3.2-3.37991-.539022.296888-2.156089.891377-3.6608 1.038932.965689-.521244 2.396444-2.228266 2.749867-3.585777" fill="#FFF" class="style-scope yt-icon"></path>
                    </g>
                  </g></svg>
                  Twitter
                </network>
              </q-btn>
            </div>
          </socialSharing>

          <q-input
            v-if="opened && currentVideo"
            class="q-ma-sm text-primary"
            style="color:black"
            white
            readonly
            v-model="url"
            :after="[
              {
                icon: 'file_copy',
                content: true,
                handler () {
                  copyLink(url)
                }
              }
            ]"
          />
        </div>
      </div>
    </q-modal-layout>
  </q-modal>
</template>
<script>
import socialSharing from 'vue-social-sharing'
import { mapGetters, mapActions } from "vuex";
export default {
  components: {
    'socialSharing': socialSharing,
  },
  methods:{
    ...mapActions({
      cancelShare: 'general/cancelShare',
      copyLink: 'video/copyLink',
    })
  },
  computed: {
    ...mapGetters({
      isShare:'general/isShare',
      currentVideo: 'video/current',
    }),
    url() {
      return window.location.href
    },
    embed() {
      return '<iframe ' +
        'src="https://player.vimeo.com/video/283154925?badge=0&autopause=0&player_id=0&app_id=138559" ' +
        'width="1920" height="1080" frameborder="0" ' +
        'title="' + this.currentVideo.title + '" allow="autoplay; ' +
        'fullscreen" allowfullscreen></iframe>'
    },
    opened: {
      get() { return this.isShare },
      set(state) {
        if (!state) {
          this.cancelShare()
        }
      }
    }
  },
  watch: {
    opened() {
    }
  },
  data () {
    return {
      socials: [
        {
          network: 'facebook',
          label: 'Facebook',
        },
        {
          network: 'googleplus',
          label: 'Google +',
        },
        {
          network: 'twitter',
          label: 'Twitter',
        }
      ]
    }
  }
}
</script>
<style lang="scss">
.social-sharing {
  display: flex;
  justify-content: space-around;
  flex-wrap: wrap;
  @media (min-width: 768px){
    flex-wrap: nowrap;
  }
  .q-btn{
    flex: 1 40%;
    svg{
      pointer-events: none;
      display: block;
      width: 60px;
      height: 60px;
      padding-bottom: 6px;
      margin: 0 auto;
      .st0{fill:#41B1DE;}
      .st1{fill:#3A9AC4;}
      .st2{fill:#FFFFFF;}
    }
    .q-btn-inner {
      span {width: 100%;}
    }
  }
}
</style>
